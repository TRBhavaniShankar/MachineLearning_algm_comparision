...
for i in range(correlation.shape[0]):
    for j in range(i + 1, correlation.shape[0]):
        if correlation[i, j] >= 0.15:  # Features below this threshold value are eliminated
            if columns[j]:
                columns[j] = False
...
meanprecition = np.take(meanPrecScore, np.argmin(precRank))
...
modelChosen = KNeighborsClassifier(n_neighbors=best_parameters['n_neighbors'],metric=best_parameters['metric'],weights=best_parameters['weights'])
modelChosen.fit(X,Y)
